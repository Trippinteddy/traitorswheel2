(() => {
  ...
  function renderBoard(finalWinnerIndex = null) {
    const choiceIndex = finalWinnerIndex === null ? 0 : finalWinnerIndex;
    const choice = state.choices[choiceIndex];

    currentBoardChoices = [choiceIndex];
    elements.reelBoard.replaceChildren();

    const tile = document.createElement("div");
    tile.className = "reel-tile";
    if (finalWinnerIndex !== null) tile.classList.add("winner");
  ...
  function shuffleBoard() {
    const choiceIndex = randomChoiceIndex();
    const choice = state.choices[choiceIndex];

    currentBoardChoices = [choiceIndex];

    let tile = elements.reelBoard.firstElementChild;
    if (!tile) {
      renderBoard(choiceIndex);
      tile = elements.reelBoard.firstElementChild;
    }
